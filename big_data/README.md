...
数据库表中每一列都和主键相关，而不是只与部分主键相关，针对联合主键。主键列和非主键列遵循完全函数依赖关系。
(确保表中的每列都和主键完全函数依赖)
...
索引是在表的列上创建。所以关键点是索引包含一个表中列的值，并且这些值存储在一个数据结构中。索引是一种数据结构 。
...
### ![#1589F0](https://placehold.it/15/1589F0/000000?text=+) `HIVE分区partition`

在执行查询语句时候通常会扫描整个表内容，会做很多无用的工作。分区是在创建的时候可以用partition by来创建分区表。物理表会根据指定的值（列名-值）创建子目录，子目录内会存储对应这个值下面的数据。（分区的字段不存储到目录下的数据文件中，但是查询时候没有区别）。「避免Hive全表扫描，提升查询效率」

好处：提高查询速度，当使用where子句查询时候只扫描特定的目录。

通常有两种类型分区：静态，动态。静态在加载数据时候显式指定分区列。而动态是在数据推送到hive后，hive决定哪个值到哪个分区。

### ![#1589F0](https://placehold.it/15/1589F0/000000?text=+) `HIVE分区和分桶的区别`

- 分区: `是以字段的形式在表结构中存在`，通过 describe table 命令可以查看到字段存在，但是该字段不存放实际的数据内容，仅仅是分区的表示（伪列）。
- 分桶:对于表（table）或者分区，Hive 可以进一步组织成桶，也就是说桶是更为细粒度的数据范围划分。Hive 也是针对某一列进行桶的组织。`Hive 采用对列值哈希`，然后除以桶的个数求余的方式决定该条记录存放在哪个桶当中。实际使用比较少。如：我们要对两张在同一列上进行了分桶操作的表进行JOIN操作的时候，只需要对保存相同列值的桶进行JOIN操作即可。同时分桶也能让取样（Sampling）更高效。
